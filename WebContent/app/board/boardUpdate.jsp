...
<body>
	<div class="container">
		<!-- 작성완료 경로 처리하기 -->
		<!-- 첨부파일을 첨부할 때는 인코딩 타입을 enctype="multipart/form-data" 설정해줘야하고, 
      		method="post" 방식으로 고정 해줘야 한다. -->
		<form id="write-form" action="${pageContext.request.contextPath}/board/boardUpdateOk.bo" method="post"
			enctype="multipart/form-data">
			<input type="hidden" name="boardNumber" value="${board.getBoardNumber()}">
			<h1>글쓰기</h1>
			<div class="form-group">
				<label for="title">제목</label> 
				<input type="text" id="title" name="boardTitle" value="${board.getBoardTitle()}" required />
			</div>
			<div class="form-group">
